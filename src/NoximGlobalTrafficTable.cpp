	...
	while (!fin.eof()) {
		...
		if (line[0] != '\0') {
			if (line[0] != '%') {
				...
				int params = sscanf(line, "%d %d %f %f %d %d %d", &src, &dst,
				...
				if (params >= 2) {
					...
					if (params >= 6 && t_off >= 0) {
						...
						communication.t_off = DEFAULT_RESET_TIME
					...
					if (params >= 7 && t_period > 0) {
						...
						communication.t_period = DEFAULT_RESET_TIME
								+ NoximGlobalParams::simulation_time;
...
double NoximGlobalTrafficTable::getCumulativePirPor(const int src_id,
		const int ccycle, const bool pir_not_por,
