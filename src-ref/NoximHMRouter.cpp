...
int NoximRouter::selectionNoP(const vector<int>&directions,
	...
	direction_selected = equivalent_directions[rand()
			% equivalent_directions.size()];
...
vector<int> NoximRouter::routingTableBased(const int dir_in,
		const NoximCoord & current, const NoximCoord & destination) {
	NoximAdmissibleOutputs ao = routing_table.getAdmissibleOutputs(dir_in,
			coord2Id(destination));

	if (ao.size() == 0) {
		cout << "dir: " << dir_in << ", (" << current.x << "," << current. y
				<< ") --> " << "(" << destination.x << "," << destination. y
				<< ")" << endl << coord2Id(current) << "->" << coord2Id(
				destination) << endl;
	}
...
void NoximRouter::configure(const int _id, const double _warm_up_time,
