...
void NoximNoC::buildMesh() {
	...
	for (int i = 0; i < NoximGlobalParams::mesh_dim_x; i++) {
		for (int j = 0; j < NoximGlobalParams::mesh_dim_y; j++) {
			...
			t[i][j]->free_slots_neighbor[DIRECTION_EAST](
					free_slots_to_west[i + 1][j]);
	...
	for (int i = 0; i < NoximGlobalParams::mesh_dim_x; i++) {
		...
		t[i][NoximGlobalParams::mesh_dim_y - 1]->r->reservation_table.invalidate(
				DIRECTION_SOUTH);
	}
	for (int j = 0; j < NoximGlobalParams::mesh_dim_y; j++) {
		t[0][j]->r->reservation_table.invalidate(DIRECTION_WEST);
		t[NoximGlobalParams::mesh_dim_x - 1][j]->r->reservation_table.invalidate(
				DIRECTION_EAST);
	}
}
